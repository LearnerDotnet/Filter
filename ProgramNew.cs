...
namespace ConsoleApplication1
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            IQueryable<vmAllServerHotFix> queryHotFix = (from i in servers
            ...
            Filter filterVariables= new Filter(){ DeploymentFilter=queryHotFix.GroupBy(x => x.intDeploymentID).Select(x => 
                                                        new vmAllServerHotFix(){ 
                                                            strDeploymentName= x.FirstOrDefault().strDeploymentName,
                                                             intDeploymentID= x.FirstOrDefault().intDeploymentID,
                                                              intDeploymentType= x.FirstOrDefault().intDeploymentType,
                                                        }).ToList(),
                                       PackageFilter=queryHotFix.GroupBy(x => x.intPackageID).Select(x => 
                                                        new vmAllServerHotFix(){ 
                                                            strPackageName= x.FirstOrDefault().strPackageName,
                                                             intPackageID= x.FirstOrDefault().intPackageID,
                                                        }).ToList()
            };
    ...
    class Filter
    {
        public Filter()
        {
            DeploymentFilter=new  List<vmAllServerHotFix>();
            PackageFilter=new  List<vmAllServerHotFix>();
        }
        public IEnumerable<vmAllServerHotFix> DeploymentFilter{ get; set; }
        public IEnumerable<vmAllServerHotFix> PackageFilter { get; set; }
    }
}
